...
export const useSSE = (workspaceId: string | null, onMessage: OnMessage) => {
  ...
  useEffect(() => {
    ...
    es.addEventListener('event_created', handleEvent('created'));
    es.addEventListener('event_update', handleEvent('update'));
    es.addEventListener('event_progress', handleEvent('progress'));
